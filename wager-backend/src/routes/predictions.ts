...
router.post(
  ...
  async (req: Request, res: Response) => {
    // Add types
    try {
      const { wagerId, title, confidencePct, model, createdUtc } =
        req.body || {};
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Missing or invalid required fields (wagerId, title, model.provider, model.name, createdUtc)",
        });
      }
      ...
      const { data: log, error } = await supabaseAdmin
        ...
        .insert({
          ...
          ).toISOString(), // Use artifact timestamp
      ...
      if (!log) {
        ...
        console.error(
          "Supabase insert did not return log data in /predictions/record"
        );
        return res
          .status(500)
      ...
      return res.status(500).json({
        error:
          "An internal server error occurred while recording the prediction.",
      });
    }
  }
);
...
router.get("/by-wager/:wagerId", async (req: Request, res: Response) => {
  // Add types
  try {
...
router.get("/:id/artifact", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error && error.code !== "PGRST116") {
      console.error(
        `Supabase fetch error for prediction ID ${id} artifact lookup:`,
        error
      );
      throw error;
    }
    ...
    const filePath = path.resolve(
      process.cwd(), // Assumes running from project root
      artifactDir,
      `${log.wager_id}.json` // Filename based on wager_id
    );
    ...
    if (fs.existsSync(filePath)) {
      ...
      console.warn(
        `Local artifact not found for prediction ID ${id} at ${filePath}`
      );
      return res.status(404).json({
        error:
          "Artifact not available locally. Check configuration or use 0G CID if applicable.",
