    ...
    try {
      ...
      `)
        ...
        const wagers = predictions?.map((prediction) => {
            const totalVolume = prediction.prediction_sides?.reduce((sum, side) => sum + (side.total_staked || 0), 0) || 0;
            ...
            return {
                ...
                totalVolume,
                sides: prediction.prediction_sides?.map((side) => ({
                    id: side.id,
                    name: side.name,
    ...
    try {
      ...
      `)
        ...
        const wager = {
            ...
            recentBets: prediction.bets?.slice(-10).map((bet) => ({
                id: bet.id,
                amount: bet.amount,
                sideId: bet.side_id,
                user: bet.user,
...
//# sourceMappingURL=wagers.js.map