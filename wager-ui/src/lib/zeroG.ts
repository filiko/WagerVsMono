...
export function buildPredictionArtifact(input: PredictionInput) {
  const base = {
    ...
    app: {
      ...
      wager_side_mapping: input.wagerSideMapping ?? {
        left: "no",
        right: "yes",
      },
    },
...
export async function uploadToZeroG(
  ...
  // DEV FALLBACK (from original code)
 ...
 */
export async function recordPredictionTo0G(
  input: PredictionInput
): Promise<ZeroGRecordResult> {
  const { artifact, sha256 } = buildPredictionArtifact(input);
  const bytes = Buffer.from(JSON.stringify(artifact));
  const { cid } = await uploadToZeroG(bytes, {
    filename: `${input.wagerId}.json`,
  });
